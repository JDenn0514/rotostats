...
| `rotostats_warning_name_match_failure` | `replacement_level()` (league history validation, name normalization) | One or more player names in `league_history$prices` could not be matched to `projections` after Unicode NFD normalization and punctuation stripping | Check for name discrepancies between sources; only fires when `verbose = TRUE` |
