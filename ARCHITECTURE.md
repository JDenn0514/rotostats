...
3. **Mixed-case xFIP/xERA in `INVERSE_CATEGORIES`**: The constant uses `"xFIP"` and `"xERA"` (lowercase x) rather than `"XFIP"` / `"XERA"`. This is conventional FanGraphs notation and matches how these categories appear in league history data. A blanket `toupper()` normalization of the constant itself was rejected because it would diverge from the canonical names users see in data sources. The package normalizes user-supplied inputs to uppercase for membership checking but stores canonical names in their conventional casing.
